    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-black text-white px-4 py-10">
      <div className="max-w-3xl w-full bg-gradient-to-b from-red-900 to-black p-6 rounded-xl shadow-2xl border border-red-700">
        <div className="flex flex-col items-center mb-8">
          ...
          <h2 className="text-3xl md:text-4xl font-bold text-center mb-2">Network Restriction Detected</h2>
          <div className="w-24 h-1 bg-red-600 rounded mb-6"></div>
        </div>
        
        <div className="space-y-6 text-center">
          <p className="text-xl">
            We've detected that you're using <span className="font-bold text-red-500">Jio network</span>, which is currently blocking access to our movie database (TMDB).
          </p>
          
          <div className="bg-black bg-opacity-50 p-4 rounded-lg border border-red-800">
            <h3 className="text-xl font-semibold mb-3">To enjoy the full experience, please try:</h3>
            <ul className="space-y-3 text-left list-none mx-auto max-w-md">
              <li className="flex items-center">
                ...
                <span>Switching to <strong>Airtel</strong>, <strong>Vi</strong>, or another network provider</span>
              </li>
              <li className="flex items-center">
                <span className="bg-red-600 rounded-full p-1 mr-3">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                ...
                <span>Using a <strong>VPN service</strong> to bypass network restrictions</span>
              </li>
              <li className="flex items-center">
                <span className="bg-red-600 rounded-full p-1 mr-3">
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                ...
                <span>Connecting to a different <strong>WiFi network</strong></span>
              </li>
            </ul>
          </div>
          
          <p className="text-lg italic">
            You can still use the GPT Search feature, but movie results may not display properly.
          </p>
...
const Browse = () => {
  ...
  useEffect(() => {
    ...
    const handleFetchError = (event) => {
      if (event.type === 'error' && event.target instanceof XMLHttpRequest) {
        if (event.target.responseURL && event.target.responseURL.includes('themoviedb.org')) {
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (apiErrorCount >= 3 && hasNoMovieData && !showGptSearch) {
  ...
  }, [nowPlayingMovies, popularMovies, topRatedMovies, upcomingMovies, showGptSearch, apiErrorCount]);
  ...
  if (networkError && !showGptSearch && hasAttemptedLoad) {
